...
class LatticePlanner: 
    ...
    def plan(self, ego_state, starting_block, observation, traffic_light_data):
        ...
        #print(np.array(candidate_paths).shape)
    ...
    def plan_multiple(self, ego_state, starting_block, observation, traffic_light_data, top_k=5, diversity_threshold=1.0):
        # 1. Aday yolları ve engelleri al (Orijinal yapı)
        edges = self.get_candidate_edges(starting_block, ego_state)
        candidate_paths = self.get_candidate_paths(edges)
        ...
        # Güncellenmiş generate_paths sayesinde yüzlerce yanal kaydırılmış yörünge geliyor
        ...
        # 3. SIRALAMA VE MEKANSAL FİLTRELEME (Non-Maximum Suppression)
        scored_paths.sort(key=lambda x: x[0]) 
        
        ref_paths = []
        selected_geometries = [] # Çeşitliliği ölçmek için seçilmiş yolların hafızası
        
        for cost, path_geom in scored_paths:
            if len(ref_paths) >= top_k:
                break
                
            # YENİ: Mekansal Çeşitlilik Kontrolü
            is_diverse = True
            for sel_geom in selected_geometries:
                min_len = min(len(path_geom), len(sel_geom))
                if min_len > 10:
                    # İki yörünge arasındaki X ve Y farklarının öklid (L2) uzaklığını hesapla
                    distances = np.linalg.norm(path_geom[:min_len, :2] - sel_geom[:min_len, :2], axis=1)
                    # Eğer iki rota arasındaki "maksimum" sapma threshold'dan küçükse (örn 1.0m) bunlar aynı rotanın klonudur
                    if np.max(distances) < diversity_threshold: 
                        is_diverse = False
                        break
                        
            # Sadece fiziksel olarak farklı bir rota çiziyorsa listeye kabul et
            if is_diverse:
    ...
    def generate_paths2(self, ego_state, paths):
        '''Generate paths from state lattice with Explicit Lateral Offsets'''
        new_paths = []
        ego_state_coords = ego_state.rear_axle.x, ego_state.rear_axle.y, ego_state.rear_axle.heading
        
        # YENİ: Yanal Çeşitlilik Değerleri (Şerit Merkezi, 1.2m Sol, 1.2m Sağ)
        ...
        for _, (path_len, dist, path, path_polyline) in paths:
            ...
            for j, state in enumerate(target_states):
                for lat_offset in lateral_offsets: # YENİ: Her hedef için yanal varyasyonlar üret
                    # Hedef noktayı aracın heading (yönelim) açısına göre yanal (dik) olarak kaydır
                    ...
                    # İkinci aşama (düz yol) için de polyline'ı aynı miktarda kaydır ki yörünge kırılmasın
